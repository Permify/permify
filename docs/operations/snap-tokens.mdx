...
Snap Tokens are used in endpoints to represent the snapshot and get fresh results from the APIs. They are mainly used in [Write API] and [Check API]. 

The general workflow for using a snap token is getting the snap token from the response of the Write API request - basically when writing a relational tuple - then mapping it with the resource. One way of doing that is storing the snap token in an additional column in your relational database.
