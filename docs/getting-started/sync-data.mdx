...
These relational tuples and attributes represent your authorization data. 

Permify stores your data in a database you prefer. You can configure the database when running Permify Service using [configuration options](../setting-up/configuration).

Stored data is queried and utilized in Permify APIs, including the check API, which is an access control check request used to determine whether a user's action is authorized.
...
Relationships and attributes can be created with a simple API call. Since these attributes and relations are live instances, meaning they can be affected by specific user actions within the application, they should be created/deleted with a simple Permify API call at runtime.
