...
<Note>
Want to walk through this guide 1x1 rather than docs ? [schedule a call with a Permify engineer](https://www.permify.co/book-demo).
</Note>
...
We have 2 entities, these are **"user"** and **"organization"**. Entities represent your main tables. We strongly advise naming entities the same as your original database entities. 

Let's roll back to our example, 

- The `user` entity represents users. This entity is empty because it's only responsible for referencing users.

- The `organization` entity has its own relations (`admin` and `member`) which are related to the user entity. This entity also has 2 actions, respectively:
  - Organization member and admin can view files.
  - Only admins can edit files.

<Note>
For implementation's sake we'll not dive deeper into modeling but you can find more information about modeling in the [Modeling Authorization with Permify] section. Also you can check out the [Real World Examples] section to better understand some familiar use cases modeled with Permify Schema. 
...
After you completed configuration of your authorization model via Permify Schema, it's time to add authorization data to see Permify in action. 
...
Finally we're ready to control authorization. Access decision results are computed according to relational tuples and the stored model, [Permify Schema] action conditions.

Let's get back to our example and perform an example access check via [Check API]. We want to check whether a specific user has access to view files in an organization.
...
See the [Access Control Check] section to learn how access checks work and how access decisions are evaluated in Permify.

[Access Control Check]: ../../api-reference/permission/check-api

## Need any help?

Our team is happy to help you get started with Permify. If you struggle with installation or have any questions, [schedule a consultation call with one of our account executives](https://www.permify.co/book-demo). Alternatively you can join our [discord community](https://discord.gg/permify) to discuss.
