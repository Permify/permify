...
So due to this, at some point, most companies tend to design these systems as abstract entities, such as a centralized engine, that caters to apps that have many services. But it's not an easy process for [several reasons](#building-an-centralized-authorization-is-hard).
...
**Permify** is a **centralized authorization service** that offers a variety of binding and crafting options to secure your applications. It works at runtime and responds to all authorization questions from any of your apps.
...
So for a centralized authorization service to operate smoothly, this system needs to be fast, consistent, and available at all times.
...
Products quickly change due to never-ending user requirements as the company scales. It's so common that the oldest authorization systems will fall short and need to be changed down the road.
...
After it's tested and ready to go, you can switch to a new version of your model without breaking a sweat.
