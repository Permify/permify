...
`
	...
	Context("Drive Sample: Check", func() {
		It("Drive Sample: Case 1", func() {
			...
			relationshipReaderForLookupEngine := new(mocks.RelationshipReader)
			...
			relationshipReaderForLookupEngine.On("GetUniqueEntityIDsByEntityType", "t1", "doc", token.NewNoopToken().Encode().String()).Return([]string{"1", "2"}, nil).Times(1)

			checkEngine = NewCheckEngine(keys.NewNoopCheckEngineKeys(), schemaReader, relationshipReader)
			lookupEntityCommand := NewLookupEntityEngine(checkEngine, relationshipReaderForLookupEngine)
			...
			response, err = lookupEntityCommand.Run(context.Background(), req)
			Expect(err).ShouldNot(HaveOccurred())
