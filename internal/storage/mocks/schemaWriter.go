package mocks

import (
	"context"

	"github.com/stretchr/testify/mock"

	"github.com/Permify/permify/internal/storage"
)

// SchemaWriter is an autogenerated mock type for the SchemaWriter type
type SchemaWriter struct {
	mock.Mock
}

// WriteSchema - Write Schema to repository
func (_m *SchemaReader) WriteSchema(ctx context.Context, definitions []storage.SchemaDefinition) (err error) {
	ret := _m.Called(definitions)

	var r0 error
	if rf, ok := ret.Get(1).(func(context.Context, []storage.SchemaDefinition) error); ok {
		r0 = rf(ctx, definitions)
	} else {
		if e, ok := ret.Get(1).(error); ok {
			r0 = e
		} else {
			r0 = nil
		}
	}

	return r0
}
